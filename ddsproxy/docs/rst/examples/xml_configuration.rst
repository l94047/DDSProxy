...
Loading an XML file or setting the raw xml file in the |ddsproxy| YAML configuration file allows to load such profiles.
...
Execute |ddsproxy| with this configuration file (available in ``<path/to/ddsproxy_tool>/share/resources/configurations/examples/xml.yaml``).
Once the |ddsproxy| is running, messages from *publisher* in domain 0 will be forwarded by the Router to the *subscriber* in domain 1, that will print them in ``stdout``.
...
Execute |ddsproxy| with this configuration file (available in ``<path/to/ddsproxy_tool>/share/resources/configurations/examples/xml.yaml``).
Once the |ddsproxy| is running, messages from *talker* in domain 0 will be forwarded by the Router to the *listener* in domain 1, that will print them in ``stdout``.
