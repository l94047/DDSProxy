...
Execute the |ddsproxy| with the configuration file available at ``<path/to/ddsproxy_tool>/share/resources/configurations/examples/forwarding_routes.yaml``.
Once the |ddsproxy| is running, messages from the *publisher* in domain ``0`` will be forwarded by the |ddsproxy| to the *subscriber* in domain ``1``, that will print them in ``stdout``.
...
Execute the |ddsproxy| with this configuration file (available in ``<path/to/ddsproxy_tool>/share/resources/configurations/examples/forwarding_routes.yaml``).
Once the |ddsproxy| is running, nothing should happen, since there are no generic forwarding routes from domain ``1`` to domain ``0``.
...
Execute the |ddsproxy| with the configuration file available at ``<path/to/ddsproxy_tool>/share/resources/configurations/examples/forwarding_routes.yaml``.
Once the |ddsproxy| is running, messages from the *publisher* in domain ``1`` on topic ``Circle`` will be forwarded by the |ddsproxy| to the *subscriber* in domain ``0``, that will print them in ``stdout``.
...
Execute the |ddsproxy| with this configuration file (available in ``<path/to/ddsproxy_tool>/share/resources/configurations/examples/forwarding_routes.yaml``).
Once the |ddsproxy| is running, nothing should happen, since there are no topic forwarding routes on topic ``Circle`` from domain ``0`` to domain ``1``.
