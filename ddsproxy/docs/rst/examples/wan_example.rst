...
``<path/to/ddsproxy_tool>/share/resources/configurations/examples/wan_client.yaml``
...
communication features of the |ddsproxy|.

Host *H*:sub:`A`
----------------

This host runs the |ddsproxy| WAN Server, which will wait for other WAN Clients to connect to it.
Execute |ddsproxy| using file ``<path/to/ddsproxy_tool>/share/resources/configurations/examples/wan_server.yaml``.
...
WAN in |ddsproxy|.
Refer to this :ref:`section <user_manual_user_interface>` for a detailed explanation on how to execute the |ddsproxy|.

First of all, execute a ROS 2 ``demo_nodes_cpp`` *listener* in domain ``0``.
This listener will discover the Simple Participant in the |ddsproxy|, but will not receive any data yet.
...
This host runs the |ddsproxy| WAN Client, which will connect to the previously launched WAN Server.
Execute |ddsproxy| using file ``<path/to/ddsproxy_tool>/share/resources/configurations/examples/wan_client.yaml``.
...
After executing both |ddsproxy| applications in both hosts, and *talker* and *listener* applications,
...
Remember that the Participants in every |ddsproxy| could be configured as any :term:`Participant Kind`,
