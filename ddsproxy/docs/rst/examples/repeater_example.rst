...
A |ddsproxy| could work as a :term:`TURN` **Repeater** to forward messages by the same Participant.
...
The Repeater admits as many edge |ddsproxy| connections as desired, so several |ddsproxy| can use the same
...
This host runs the |ddsproxy| Repeater Server, which will receive data from both edges and redirect the messages
between them.
Execute |ddsproxy| using file
``<path/to/ddsproxy_tool>/share/resources/configurations/examples/repeater.yaml``.
...
These hosts run the |ddsproxy| WAN Clients, which will connect to the previously launched Repeater Server.
Execute |ddsproxy| using file
...
After executing the three |ddsproxy| applications in hosts, and *talker* and *listener* applications,
