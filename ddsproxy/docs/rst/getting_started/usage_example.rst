...
====================
A configuration file is all that is required in order to run a |ddsproxy| instance. In a nutshell, each router will
...
.. literalinclude:: ../../resources/getting_started/client-ddsproxy.yaml
    :language: yaml
    :lines: 5-6
...
.. literalinclude:: ../../resources/getting_started/server-ddsproxy.yaml
    :language: yaml


Router execution
================
Now, with the configuration files ready, launching a |ddsproxy| instance is as easy as executing the following command:

.. code-block:: bash

    ddsproxy -c config-file.yaml
...
|ddsproxy| supports the dynamic addition/deletion of topics at runtime (see
...
information on how to configure and set up a router, as well as to discover multiple scenarios where |ddsproxy| may
