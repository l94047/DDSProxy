...
eProsima distributes a Docker image of |ddsproxy| with Ubuntu 20.04 as base image.
This image launches an instance of |ddsproxy| that is configured using a *YAML* configuration file provided by the user
and shared with the Docker container.
The steps to run |ddsproxy| in a Docker container are explained below.
...
#.  Download the compressed Docker image in ``.tar`` format from the
    ...
    It is strongly recommended to download the image corresponding to the latest version of |ddsproxy|.
...
#.  Extract the image by executing the following command:
    ...
    .. code-block:: bash
        ...
        load ubuntu-ddsproxy:<version>.tar

    where ``version`` is the downloaded version of |ddsproxy|.

    |br|

#.  Build a |ddsproxy| configuration YAML file on the local machine.
    This will be the |ddsproxy| configuration file that runs inside the Docker container.
...
#.  Run the Docker container executing the following command:
    ...
    After executing the previous command you should be able to see the initialization traces from the |ddsproxy|
    running in the Docker container.
    If you want to terminate the application gracefully, just press ``Ctrl+C`` to stop the execution of |ddsproxy|.
