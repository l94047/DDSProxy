...
A |ddsproxy| is configured by a *.yaml* configuration file.
This *.yaml* file contains all the information regarding the |ddsproxy| configuration, such as topics filtering and :term:`Participants <Participant>` configurations.
...
Another way of loading these XML configurations is using the |ddsproxy| yaml configuration.
...
.. note::
    ...
    The |ddsproxy| does not modify any XML configuration in a :ref:`user_manual_participants_xml`.
...
The |ddsproxy| will create the DataWriters and DataReaders for these topics in the |ddsproxy| initialization.
...
The |ddsproxy| automatically detects the topics that are being used in a DDS Network.
The |ddsproxy| then creates internal DDS :term:`Writers<DataWriter>` and :term:`Readers<DataReader>` for each participant in each topic, and forwards the data published on each topic.
The |ddsproxy| allows filtering DDS :term:`Topics<Topic>` to allow users to configure the DDS :term:`Topics<Topic>` that must be forwarded.
These data filtering rules can be configured under the ``allowlist`` and ``blocklist`` tags.
If the ``allowlist`` and ``blocklist`` are not configured, the |ddsproxy| will forward all the data published on the topics it discovers.
...
In this example, the data in the topic ``AllowedTopic1`` with type ``Allowed`` and the data in the topic ``AllowedTopic2`` with any type will be forwarded by the |ddsproxy|.
...
By default, the depth of every RTPS History instance is :code:`5000`, which sets a constraint on the maximum number of samples a |ddsproxy| instance can deliver to late joiner Readers configured with ``TRANSIENT_LOCAL`` `DurabilityQosPolicyKind <https://fast-dds.docs.eprosima.com/en/latest/fastdds/dds_layer/core/policy/standardQosPolicies.html#durabilityqospolicykind>`_.
Its value should be decreased when the sample size and/or number of created endpoints (increasing with the number of topics and |ddsproxy| participants) are big enough to cause memory exhaustion issues.
...
``specs`` supports a ``remove-unused-entities`` **optional** value that configures the deletion of unused internal entities in the |ddsproxy|.
By default, unused internal entities are *not* removed.
Thus, when the |ddsproxy| discovers a Subscriber (by default; see :ref:`Discovery Trigger <user_manual_configuration_discovery_trigger>`), the |ddsproxy| creates entities in all of its participants, and these entities stay up even after the Subscriber disconnects.
...
By setting the ``remove-unused-entities`` option to ``true``, the internal entities of the |ddsproxy| would be removed, and the *DDS Routers* would stop communicating and free up the bandwidth.
...
``specs`` supports a ``discovery-trigger`` **optional** value that configures what type of external entity triggers the creation/removal of entities in the |ddsproxy|.
...
.. warning::
  When the |ddsproxy| creates internal entities triggered by the discovery of a writer (i.e. the ``discovery-trigger`` is either ``writer`` or ``any``), the |ddsproxy| will create its internal entities with the writer's :ref:`Topic QoS <user_manual_configuration_topic_qos>`, and, therefore, the QoS of the communication between the external entities and the |ddsproxy| may differ from the QoS of the communication without the |ddsproxy|.
...
In the following configuration example, the |ddsproxy| will create two
...
i.e. the data must reach the |ddsproxy| and this will forward the data.
...
Using tag ``id``, the GuidPrefix will be calculated arbitrarily using a default |ddsproxy| GuidPrefix.
...
.. note::
    ...
    In the current version of the |ddsproxy| only ids in the range 0 to 256 are allowed.
...
The |ddsproxy| is capable of establishing different internal routes between its participants.
