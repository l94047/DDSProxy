...
After having sourced the |ddsproxy| workspace, execute the following command in order to validate a YAML configuration
file:
...
.. code-block:: bash
    ...
    ddsproxy_yaml_validator --config-file ddsproxy-config.yaml --schema schema.json
