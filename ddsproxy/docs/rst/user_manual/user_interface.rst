...
|eddsproxy| is a user application executed from the command line and configured through a YAML configuration file.
...
|eddsproxy| depends on |fastdds| ``fastrtps`` and ``fastcdr`` libraries.
...
The |ddsproxy| application supports several input arguments:
...
.. code-block:: console
    ...
    -d --debug          Set log verbosity to Info (Using this option with --log-filter and/or --log-verbosity will head to undefined behaviour).
        --log-filter     Set a Regex Filter to filter by category the info and warning log entries. [Default = "DDSPROXY"].
...
This argument enables the |ddsproxy| logs so the execution can be followed by internal debugging information.
This argument sets :ref:`user_manual_user_interface_log_verbosity_argument` to ``info`` and
:ref:`user_manual_user_interface_log_filter_argument` to ``DDSPROXY``.
...
A |ddsproxy| **requires** one and only one *YAML* configuration file as the operation of this application is
...
This *YAML* configuration file must be passed as argument to the |ddsproxy| when executed.
If no configuration file is provided as argument, the |ddsproxy| will attempt to load a file named
...
The topics that the |ddsproxy| is routing could be changed at runtime.
...
So, if a topic has been active before, the Writers and Readers will still be present in the |ddsproxy| and will still
receive data.

There exist two methods to reload the list of allowed topics, an active and a passive one.
Both methods work over the same configuration file with which the |ddsproxy| has been initialized.


File Watcher
^^^^^^^^^^^^

A File Watcher is a process that runs in the background and tracks changes in the |ddsproxy| configuration file.
Every time the file is changed, the OS sends a notification, and the File Watcher listens such notification
and interacts with the |ddsproxy| in order to reload the topics.
This event occurs every time the configuration file is saved.

FileWatcher is used in every |ddsproxy| execution by default.
...
Log module of |ddsproxy| uses the |fastdds| logging module.
...
.. note::
    ...
    For ``INFO`` logs to be compiled, the |ddsproxy| must have been compiled with CMake option ``CMAKE_BUILD_TYPE=Debug``,
...
In order to stop a |ddsproxy| application, use one of the following OS signals:
...
Send an interruption ``SIGTERM`` signal *(signal value 15)* to the process by executing the command ``kill <pid>`` in a different terminal, where ``<pid>`` is the id of the process running the |ddsproxy|.
