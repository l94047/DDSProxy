...
DDS Router :term:`Participant` is a |ddsproxy| entity that works as an interface between a network and
the core of the router.
Participants are the main elements inside the |ddsproxy| functionality.
...
Each Participant is uniquely identified by a :term:`Participant Name` in a |ddsproxy| execution and has a
...
It is an alphanumeric string that uniquely identifies a Participant in a |ddsproxy| execution.
...
configurations has a unique Participant Name that should not be repeated in a |ddsproxy| execution.
