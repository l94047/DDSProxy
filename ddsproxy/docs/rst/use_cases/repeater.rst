...
A |ddsproxy| could work as a :term:`TURN` **Repeater**.
This means that a |ddsproxy| can be used to repeat messages between networks.
...
This |ddsproxy| configuration is very simple, as all it needs is a :ref:`user_manual_participants_wan` and
adding to its configuration the tag `repeater: true`.
There could be more Participants in this |ddsproxy| and topic filtering is also available.
