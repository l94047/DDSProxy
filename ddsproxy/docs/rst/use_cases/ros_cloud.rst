...
present throughout the |ddsproxy| design phase. In this walk-through example, we will set up both a *Kubernetes*
...
a |ddsproxy| instance at each side of the communication.

.. figure:: /rst/figures/ddsproxy_overview_wan.png

Local setup
===========
The local instance of |ddsproxy| (local router) only requires to have a
...
.. literalinclude:: ../../resources/use_cases/ros_cloud/local-ddsproxy.yaml
    :language: yaml
...
.. code-block:: bash
    ...
    ddsproxy --config-path local-ddsproxy.yaml
...
Kubernetes setup
================
Two different deployments will be used for this example, each in a different |k8s| pod. The |ddsproxy| cloud instance
...
|ddsproxy|, which can be accomplished by providing the following
:download:`Dockerfile <../../resources/use_cases/ros_cloud/Dockerfile_ddsproxy>`. If willing to see log messages in
``STDOUT``, use :download:`Dockerfile <../../resources/use_cases/ros_cloud/Dockerfile_ddsproxy_logon>` instead.
Assuming the name of the generated Docker image is ``ddsproxy:main``, the cloud router will then be deployed with the
following settings:

.. literalinclude:: ../../resources/use_cases/ros_cloud/ddsproxy.yaml
    :language: yaml
