...
The instructions for installing the |ddsproxy| application from sources and its required
...
|ddsproxy| depends on *eProsima Fast DDS* library and certain Debian packages.
This section describes the instructions for installing |ddsproxy| dependencies and requirements in a Linux
...
The installation of |ddsproxy| in a Linux environment from sources requires the following tools to be
...
These packages provide the tools required to install |ddsproxy| and its dependencies from command line.
...
By default, |ddsproxy| does not compile tests.
...
It is also possible to clone the Gtest_ Github repository into the |ddsproxy| workspace and compile it with colcon_
...
|ddsproxy| has the following dependencies, when installed from sources in a Linux environment:
...
this library when building the |ddsproxy| application by using the command:

.. code-block:: bash

    source <fastdds-installation-path>/install/setup.bash

In other case, just download *Fast DDS* project from sources and build it together with |ddsproxy| using colcon
...
#.  Create a :code:`DDS-Router` directory and download the :code:`.repos` file that will be used to install
    |ddsproxy| and its dependencies:

    .. code-block:: bash

        mkdir -p ~/DDS-Router/src
        cd ~/DDS-Router
        wget https://raw.githubusercontent.com/eProsima/DDS-Router/main/ddsproxy.repos
        vcs import src < ddsproxy.repos
    ...
    .. note::
        ...
        It is just needed to download and build the |ddsproxy| project having sourced its dependencies.
...
This section explains how to compile |ddsproxy| with CMake_, either
...
#.  Create a :code:`DDS-Router` directory where to download and build |ddsproxy| and its dependencies:
...
#.  Once all dependencies are installed, install |ddsproxy|:
    ...
    .. code-block:: bash
        ...
        cmake ~/DDS-Router/src/ddsproxy/ddsproxy_core -DCMAKE_INSTALL_PREFIX=~/DDS-Router/install -DCMAKE_PREFIX_PATH=~/DDS-Router/install
        cmake --build . --target install
        ...
        cmake ~/DDS-Router/src/ddsproxy/ddsproxy_yaml -DCMAKE_INSTALL_PREFIX=~/DDS-Router/install -DCMAKE_PREFIX_PATH=~/DDS-Router/install
        cmake --build . --target install

        # ddsproxy_tool
        cd ~/DDS-Router
        mkdir build/ddsproxy_tool
        cd build/ddsproxy_tool
        cmake ~/DDS-Router/src/ddsproxy/tools/ddsproxy_tool -DCMAKE_INSTALL_PREFIX=~/DDS-Router/install -DCMAKE_PREFIX_PATH=~/DDS-Router/install
        cmake --build . --target install
...
To install |ddsproxy| system-wide instead of locally, remove all the flags that
...
To run the |ddsproxy| application, source the *Fast DDS* library
and execute the executable file that has been installed in :code:`<install-path>/ddsproxy_tool/bin/ddsproxy`:

.. code-block:: bash

    # If built has been done using colcon, all projects could be sourced as follows
    source install/setup.bash
    ./<install-path>/ddsproxy_tool/bin/ddsproxy
