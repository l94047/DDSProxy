...
![eProsima DDS Router overall architecture](docs/rst/figures/ddsproxy_cloud_white_background.png)
...
```bash
colcon build --packages-select-regex ddsproxy
```

This repository holds several colcon packages.
These packages are:

* `ddsproxy_core`: library with the main functionality of the DDS Router.
* `ddsproxy_yaml`: library to configure a DDS Router from a YAML.
* `ddsproxy_tool`: application to execute a DDS Router from a YAML configuration file.
* `ddsproxy_docs`: package to generate the DDS Router documentation using sphinx.
* `ddsproxy_yaml_validator`: application to validate DDS Router YAML configuration files.
...
installed in `<install-path>/ddsproxy_tool/bin/ddsproxy`:
...
ddsproxy_yaml_validator --config-file ddsproxy-config.yaml
```
...
```bash
colcon test --packages-select-regex ddsproxy --event-handler=console_direct+
```
