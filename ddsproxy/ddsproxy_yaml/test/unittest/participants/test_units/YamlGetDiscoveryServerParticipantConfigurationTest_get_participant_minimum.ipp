...
#include <ddsproxy_core/types/ParticipantKind.hpp>

#include <ddsproxy_yaml/testing/generate_yaml.hpp>
...
{
    for (ddsproxy::core::types::ParticipantKind kind : ddsproxy::core::types::VALUES_ParticipantKind)
    {
        for (uint32_t i = 0; i < ddsproxy::yaml::testing::TEST_ITERATIONS; i++)
        {
            ddspipe::core::types::ParticipantId id = ddspipe::core::testing::random_participant_id(i);
            for (uint32_t j = 0; j < ddsproxy::yaml::testing::TEST_ITERATIONS; j++)
            {
                ...
                ddsproxy::yaml::testing::participantkind_to_yaml(yml_participant, kind);
