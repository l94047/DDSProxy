 ...
 *
 * @return DdsProxyConfiguration
 */
DdsProxyConfiguration router_configuration(
        std::pair<
            types::ParticipantKind,
            std::shared_ptr<participants::ParticipantConfiguration>>
        participant_configuration,
...
void test_WAN_communication(
        DdsProxyConfiguration ddsproxy_client_configuration_1,
        DdsProxyConfiguration ddsproxy_client_configuration_2,
        DdsProxyConfiguration ddsproxy_repeater_configuration,
        ...
        uint32_t msg_size = DEFAULT_MESSAGE_SIZE)
{
    ...
    {
        msg_str += "Testing DdsProxy Blackbox Local Communication ...";
    }
    msg.message(msg_str);
    ...
    // Create DdsProxy entity whose WAN Participant is configured as server
    DdsProxy client_router_1(ddsproxy_client_configuration_1);
    client_router_1.start();

    // Create DdsProxy entity whose WAN Participant is configured as client
    DdsProxy client_router_2(ddsproxy_client_configuration_2);
    client_router_2.start();

    // Create DdsProxy entity whose WAN Participant is configured as repeater
    DdsProxy repeater(ddsproxy_repeater_configuration);
    repeater.start();
