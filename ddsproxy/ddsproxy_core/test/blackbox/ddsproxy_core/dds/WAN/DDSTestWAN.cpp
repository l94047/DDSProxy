        ...
        WanKind wan_kind)
{
    ...
    if (is_server(wan_kind))
    {
        ...
        tls.private_key_file = "../../resources/tls/ddsproxy.key";
        tls.certificate_chain_file = "../../resources/tls/ddsproxy.crt";
...
void test_WAN_communication(
        DdsProxyConfiguration ddsproxy_server_configuration,
        DdsProxyConfiguration ddsproxy_client_configuration,
        ...
        uint32_t msg_size = DEFAULT_MESSAGE_SIZE)
{
    ...
    DdsProxy server_router(ddsproxy_server_configuration);
    server_router.start();

    // Create DdsProxy entity whose WAN Participant is configured as client
    DdsProxy client_router(ddsproxy_client_configuration);
    client_router.start();
