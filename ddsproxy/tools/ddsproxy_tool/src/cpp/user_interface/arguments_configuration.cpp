...
#include <ddsproxy_core/library/config.h>
        ...
        eprosima::fastdds::dds::Log::Kind& log_verbosity)
{
    ...
    if (argc > 0)
    {
        ...
        {
            logError(DDSPROXY_ARGS, "ERROR: Unknown argument: <" << parse.nonOption(0) << ">." );
            option::printUsage(fwrite, stdout, usage, columns);
            return ProcessReturnCode::incorrect_argument;
        }
        ...
        for (int i = 0; i < parse.optionsCount(); ++i)
        {
            ...
            switch (opt.index())
            {
                ...
                case optionIndex::UNKNOWN_OPT:
                    logError(DDSPROXY_ARGS, opt << " is not a valid argument.");
                    option::printUsage(fwrite, stdout, usage, columns);
                    return ProcessReturnCode::incorrect_argument;
                    break;
        ...
        bool msg)
{
    ...
    if (msg)
    {
        logError(DDSPROXY_ARGS, "Option '" << option << "' required.");
    }
    return option::ARG_ILLEGAL;
}
        ...
        bool msg)
{
    ...
    if (msg)
    {
        logError(DDSPROXY_ARGS, "Option '" << option << "' requires a numeric argument.");
    }
    return option::ARG_ILLEGAL;
}
        ...
        bool msg)
{
    ...
    if (msg)
    {
        logError(DDSPROXY_ARGS, "Option '" << option << "' requires a float argument.");
    }
    return option::ARG_ILLEGAL;
}
        ...
        bool msg)
{
    ...
    if (msg)
    {
        logError(DDSPROXY_ARGS, "Option '" << option << "' requires a text argument.");
    }
    return option::ARG_ILLEGAL;
}
        ...
        bool msg)
{
    ...
    if (msg)
    {
        logError(DDSPROXY_ARGS, "Option '" << option << "' requires an existing readable file as argument.");
    }
    return option::ARG_ILLEGAL;
}
        ...
        bool msg)
{
    if (nullptr == option.arg)
    {
        if (msg)
        {
            logError(DDSPROXY_ARGS, "Option '" << option.name << "' requires a text argument.");
        }
        return option::ARG_ILLEGAL;
    }
    ...
    else if (msg)
    {
        ...
        logError(DDSPROXY_ARGS, error_msg);
    }
