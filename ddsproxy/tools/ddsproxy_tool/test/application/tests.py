...
"""
Tests for the ddsproxy executable.

Contains a package of system test for ddsproxy tool
...
USAGE = ('python3 tests.py -e <path/to/ddsproxy-executable>'
...
def test_ddsproxy_closure(ddsproxy, configuration_file, killing_signal):
    """
    Test that ddsproxy command closes correctly.
    ...
    Parameters:
    ddsproxy (path): Path to ddsproxy binary executable
    configuration_file (path): Path to ddsproxy yaml configuration file
    use_sigint (KillingSignalType): Signal to kill subprocesses

    Returns:
    0 if okay, otherwise the return code of the command executed
    """
    command = [ddsproxy, '-c', configuration_file]
